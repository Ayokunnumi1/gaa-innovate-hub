...
const DeveloperDashboard = () => {
  const { currentUser, ideas, developerInterests } = useApp();

  const interestedIdeas = ideas.filter((idea) => developerInterests.includes(idea.id));

  return (
    <div className="min-h-screen bg-[#F9FAFB] py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-[#0F3D73]">Developer Dashboard</h1>
          <p className="text-gray-600 mt-2">Welcome back, {currentUser?.name}!</p>
        </div>

        <div className="bg-white rounded-xl shadow-md p-8 mb-6">
          <h2 className="text-2xl font-bold text-[#0F3D73] mb-6">My Development Projects</h2>
          {interestedIdeas.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-600 mb-4">You haven't selected any ideas to develop yet.</p>
              <Link
                to="/marketplace"
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {interestedIdeas.map((idea) => (
                ...
                >
                  <div className="mb-3">
                    <span className="inline-block bg-[#1DBF73] text-white text-xs font-semibold px-3 py-1 rounded-full">
                      {idea.sdg}
                    </span>
                  </div>
